...
module.exports.execute = async (client, message, args) => {
    ...
    embed.setFooter("⚔ Assassins Developed by Synayzen İvar").setDescription(`${message.guild.name} sunucusunda kullanıcıların **${day}** gün boyunca yapmış olduğu ses aktifliği aşağıda detaylı olarak sıralanmıştır. Bir önceki güne gitmek için yöneticiye başvurunuz.`);
    ...
    Stat.aggregate([
        ...
        if (docs.length > 0) {
            ...
            embed.addField(`Top 10 Genel Ses İstatistiği`, usersToEmbed.map((val, index) => `\`${index + 1}.\` ${val.User}(${val.User.username}): \`${moment.duration(val.Value * (1000 * 60)).format("H [saat, ] m [dakika]")}\``).join("\n"))
