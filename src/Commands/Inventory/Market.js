...
module.exports.execute = async (client, message, args) => {
    ...
    if (args[0] == "satınal") {
        ...
        User.updateOne({Id: user.Id}, {$inc: {"Coin": money}}).exec((err, res) => {
            ...
            message.reply(`başarılı bir şekilde ${item.Name} eşyasından **${count}** adet sattın ve sana toplam **${money}** <:dolar:843433771168694282> para verildi.`);
        });
        return;
    }
    ...
    message.channel.send(new MessageEmbed().setFooter("⚔ Assassins Developed by Synayzen İvar").setDescription(list).setColor("#754f46").setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })));
}
